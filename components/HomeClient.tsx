...
export default function HomeClient({ ssrMobile }: { ssrMobile: boolean }) {
  ...
  const { isMobile, ready } = useIsMobile('(max-width: 820px)', ssrMobile)
  // Trước khi matchMedia chạy (pha mount), tin theo server để không nháy desktop.
  const mobile = ready ? isMobile : ssrMobile
